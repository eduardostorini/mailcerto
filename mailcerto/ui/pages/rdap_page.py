...
class RdapPage(QWidget):
    ...
    def init_ui(self):
        ...
        title = QLabel("WHOIS & RDAP")
        title.setObjectName("titleLabel")
        title.setStyleSheet("font-size: 18px; font-weight: bold;")
        layout.addWidget(title)

        desc = QLabel("Consulte informações de registro e eventos de propriedade do domínio via protocolo RDAP público.")
    ...
    def on_row_selected(self, index):
        ...
        if 0 <= row < len(self._current_results):
            ...
            if result.details:
                detail_content += f"<b>Metadados RDAP / WHOIS:</b><br/><pre>{result.details}</pre>"
            if result.recommendation:
                detail_content += f"<br/><b>Recomendação:</b><br/>{result.recommendation}"
            self.details_text.setText(detail_content)

    def start_rdap_analysis(self, domain: str):
        ...
        self.details_text.setText("Consultando dados RDAP... Por favor, aguarde.")
        
        self.progress_bar.setVisible(True)
        self.progress_bar.setMaximum(1)
        self.progress_bar.setValue(0)
        
        task = asyncio.create_task(self._run_rdap_check(domain))
        self.active_tasks.append(task)

    async def _run_rdap_check(self, domain: str):
        try:
            results = await check_rdap_whois(domain)
            self.progress_bar.setMaximum(len(results))
            for r in results:
                self.check_finished.emit(r)
        except Exception as e:
            self.check_finished.emit(CheckResult(
                check_id="rdap_err", category="WHOIS & RDAP", title="RDAP",
