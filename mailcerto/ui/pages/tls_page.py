...
class TLSPage(QWidget):
    ...
    def init_ui(self):
        ...
        title = QLabel("TLS & Certificados")
        title.setObjectName("titleLabel")
        title.setStyleSheet("font-size: 18px; font-weight: bold;")
        layout.addWidget(title)

        desc = QLabel("Inspecione certificados SSL/TLS da porta HTTPS (443) do domínio.")
        ...
        self.details_title = QLabel("Detalhes do Certificado")
    ...
    def on_row_selected(self, index):
        ...
        if 0 <= row < len(self._current_results):
            ...
            if result.details:
                detail_content += f"<b>Metadados do Certificado:</b><br/><pre>{result.details}</pre>"
            if result.recommendation:
                detail_content += f"<br/><b>Recomendação:</b><br/>{result.recommendation}"
            self.details_text.setText(detail_content)

    def start_tls_analysis(self, domain: str):
        ...
        self.details_text.setText("Verificando certificado SSL/TLS... Por favor, aguarde.")
        
        self.progress_bar.setVisible(True)
        self.progress_bar.setMaximum(1)
        self.progress_bar.setValue(0)
        
        task = asyncio.create_task(self._run_tls_check(domain))
        self.active_tasks.append(task)

    async def _run_tls_check(self, domain: str):
        try:
            results = await check_tls_cert(domain)
            for r in results:
                self.check_finished.emit(r)
        except Exception as e:
            self.check_finished.emit(CheckResult(
                check_id="tls_err", category="TLS & Certificados", title="TLS",
