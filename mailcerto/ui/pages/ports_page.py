...
from mailcerto.checks.network.port_scan import COMMON_PORTS, scan_single_port
...
class PortsPage(QWidget):
    ...
    def init_ui(self):
        ...
        title = QLabel("Varredura de Portas (Scan)")
        title.setObjectName("titleLabel")
        title.setStyleSheet("font-size: 18px; font-weight: bold;")
        layout.addWidget(title)

        desc = QLabel("Escanear portas TCP comuns do mercado (SMTP, HTTP, HTTPS, SSH, FTP, etc.) para identificar portas abertas no host.")
        ...
        self.details_title = QLabel("Detalhes da Porta")
    ...
    def on_row_selected(self, index):
        ...
        if 0 <= row < len(self._current_results):
            ...
            detail_content = f"<b>Porta:</b> {result.title}<br/>"
            ...
            if result.details:
                detail_content += f"<b>Detalhes do Escaneamento:</b><br/><pre>{result.details}</pre>"
            self.details_text.setText(detail_content)

    def start_ports_scan(self, domain: str):
        ...
        self.details_text.setText("Escaneando portas comuns do host... Por favor, aguarde.")
        
        # Resolve target domain to IP first
        import socket
        try:
            ip = socket.gethostbyname(domain)
        except Exception as e:
            self.details_text.setText(f"Erro ao resolver domínio para IP: {str(e)}")
            return
            
        self.progress_bar.setVisible(True)
        self.progress_bar.setMaximum(len(COMMON_PORTS))
        self.progress_bar.setValue(0)
        
        # Concurrently scan all ports
        for port, service in COMMON_PORTS.items():
            task = asyncio.create_task(self._run_single_port_scan(ip, port, service))
            self.active_tasks.append(task)

    async def _run_single_port_scan(self, ip: str, port: int, service: str):
        try:
            result = await scan_single_port(ip, port, service)
            self.check_finished.emit(result)
        except Exception as e:
            self.check_finished.emit(CheckResult(
                check_id=f"port_{port}_err", category="Portas", title=f"Porta {port}",
    ...
    def on_check_finished(self, result: CheckResult):
        ...
        # Filtra para mostrar primeiro as portas abertas (SUCCESS), depois as outras
        sorted_results = sorted(self._current_results, key=lambda x: x.status != CheckStatus.SUCCESS)
        self.results_model.update_results(sorted_results)
