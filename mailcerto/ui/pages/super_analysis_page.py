...
from mailcerto.core.analyzer import SuperAnalyzer
from mailcerto.core.models import CheckResult, CheckStatus

class SuperAnalysisPage(QWidget):
    ...
    def init_ui(self):
        ...
        title = QLabel("Super Análise Concorrente")
        title.setObjectName("titleLabel")
        title.setStyleSheet("font-size: 18px; font-weight: bold;")
        layout.addWidget(title)

        desc = QLabel("Executa todas as verificações do MailCerto em paralelo (DNS, SPF, DMARC, SMTP, SSL/TLS, Reputação, HTTP).")
        ...
        self.details_title = QLabel("Detalhes da Super Análise")
    ...
    def on_row_selected(self, index):
        ...
        if 0 <= row < len(self._current_results):
            ...
            detail_content = f"<b>Categoria:</b> {result.category}<br/>"
    ...
    def start_super_analysis(self, domain: str):
        ...
        self.details_text.setText("Iniciando varredura paralela completa... Por favor, aguarde.")
        
        self.progress_bar.setVisible(True)
        self.progress_bar.setMaximum(15) # estimativa de total de checks
        self.progress_bar.setValue(0)
        
        analyzer = SuperAnalyzer(domain)
        task = asyncio.create_task(analyzer.run_all_checks(self._emit_check_result))
        self.active_tasks.append(task)

    def _emit_check_result(self, result: CheckResult):
