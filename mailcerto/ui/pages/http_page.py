...
class HttpPage(QWidget):
    ...
    def init_ui(self):
        ...
        title = QLabel("HTTP & Segurança")
        title.setObjectName("titleLabel")
        title.setStyleSheet("font-size: 18px; font-weight: bold;")
        layout.addWidget(title)

        desc = QLabel("Valide redirecionamentos HTTP para HTTPS e analise cabeçalhos de segurança (CSP, HSTS).")
        ...
        self.details_title = QLabel("Detalhes de HTTP/Segurança")
    ...
    def start_http_analysis(self, domain: str):
        ...
        self.details_text.setText("Verificando protocolo HTTP/HTTPS e cabeçalhos de segurança... Por favor, aguarde.")
        
        self.progress_bar.setVisible(True)
        self.progress_bar.setMaximum(1)
        self.progress_bar.setValue(0)
        
        task = asyncio.create_task(self._run_http_check(domain))
        self.active_tasks.append(task)

    async def _run_http_check(self, domain: str):
        try:
            results = await check_http_security(domain)
            self.progress_bar.setMaximum(len(results))
            for r in results:
                self.check_finished.emit(r)
        except Exception as e:
            self.check_finished.emit(CheckResult(
                check_id="http_err", category="HTTP & Segurança", title="HTTP/HTTPS",
